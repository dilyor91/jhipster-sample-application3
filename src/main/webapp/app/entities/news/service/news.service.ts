...
type RestOf<T extends INews | NewNews> = Omit<T, 'postedDate'> & {
...
export class NewsService {
  ...
  protected convertDateFromClient<T extends INews | NewNews | PartialUpdateNews>(news: T): RestOf<T> {
    return {
      ...news,
      postedDate: news.postedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNews: RestNews): INews {
    return {
      ...restNews,
      postedDate: restNews.postedDate ? dayjs(restNews.postedDate) : undefined,
    };
  }
