...
type RestOf<T extends IEvents | NewEvents> = Omit<T, 'postedDate'> & {
...
export class EventsService {
  ...
  protected convertDateFromClient<T extends IEvents | NewEvents | PartialUpdateEvents>(events: T): RestOf<T> {
    return {
      ...events,
      postedDate: events.postedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvents: RestEvents): IEvents {
    return {
      ...restEvents,
      postedDate: restEvents.postedDate ? dayjs(restEvents.postedDate) : undefined,
    };
  }
